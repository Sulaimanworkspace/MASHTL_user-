...
class WebSocketService {
  ...
  private messageCallbacks: ((data: any) => void)[] = []; // ← ADDED THIS
    ...
    try {
      ...
      this.socket.on('connect', () => {
        ...
        this.socket.on('new_message', (message: any) => {
          ...
          // Forward to registered message callbacks
          this.messageCallbacks.forEach(callback => callback(message));
          console.log('🔌 Current userId:', this.currentUserId);
        });
        
        // Chat room joined confirmation
        this.socket.on('chat_room_joined', (data: any) => {
          console.log('✅ Chat room joined confirmation:', data);
          console.log(`✅ Successfully joined ${data.roomName} with ${data.memberCount} members`);
        });
      });
  ...
  public on(event: string, callback: (...args: any[]) => void): void {
    if (event === 'new_message') {
      ...
      console.log(`🔌 Adding message callback for: ${event}`);
  ...
  public off(event: string, callback?: (...args: any[]) => void): void {
    if (event === 'new_message') {
      // Handle new_message events specially through our callback system
      console.log(`🔌 Removing message callback for: ${event}`);
      if (callback) {
  ...
  public disconnect(): void {
    if (this.socket) {
      ...
      this.isConnecting = false;
      this.reconnectAttempts = 0;
      this.messageCallbacks = []; // ← ADDED THIS
    }
  }
